...
export default {
	...
	setup(props, { emit }) {
		...
		const getPages = () => {
			...
			if (
				current.value >= prevRange.value &&
				current.value <= nextRange.value &&
				props.total > 9
			) {
				...
				pages.value = [
					...setPages(1, prevRange.value),
					'...',
					...setPages(nextRange.value, props.total),
				];
			}
		};
		...
		const goTo = page => {
			...
			if (typeof page.target === 'undefined') {
				...
				go.value =
					value < 1 ? 1 : value <= props.total ? value : props.total;
