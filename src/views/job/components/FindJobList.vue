	...
	<section class="find-section">
		<div>
			<div class="invoice__list card-space">
				<div>
					<h5 class="title-lg">{{ t('commonDashboard.jobMatching') }}</h5>
				</div>
				<div class="form-row">
					<div class="form-group col-12">
						<label class="d-block"> {{ t('skill.yourSkills') }}<span>*</span> </label>
						<div class="user_skills">
							<span v-for="(skill, index) in leSkillList" :key="index" class="user_skill">
				...
				<div class="find">
					<div class="dropdown border-radius dropdown-multiselct">
						<Multiselect
							class="form-group multi-label-text"
							label="industry_name"
					...
					<div class="dropdown dropdown-multiselct">
						<Multiselect
							id="city"
							class="form-group"
					...
					<ToolTip :title="t('toolTip.Searchapplication')">
							...
							<div class="apply-for-this-section">
								<div class="apply-for-this">
									...
									<div class="card-content">
										<div class="card-text-btn">
											...
											<button class="btn-common btn-red ml-auto" @click="showLoginForm = true" v-if="!isUserLogedIn">
												<ToolTip :title="t('toolTip.applyForThisGig')">
											...
											<button
												class="btn-common btn-red ml-auto"
												@click="applyJobs(projectDetails.id)"
												v-else-if="userTypeInfo == 'jobseeker' && projectDetails.job_proposal === 0"
											>
												<ToolTip :title="t('toolTip.applyForThisGig')">
													{{ t('projectMarketPage.applyForThisGig') }}
												</ToolTip>
											...
											<button class="btn-common btn-green ml-auto" v-else-if="projectDetails.job_proposal">
								...
								<div class="d-flex flex-wrap">
									<div v-for="item in projectDetails.skills" :key="item.id" class="labels">
										<div class="labels-name btn-wrap">
...
export default {
	...
	setup() {
		...
		onMounted(() => {
			...
			// get le user skill
			store.dispatch('projectmarket/getLeSkillList');
		});
		...
		// project market list
		const projectMarketList = computed(() => store.getters['job/getMatchingJobList']);
		// city options
		...
		const leSkillList = computed(() => store.getters['projectmarket/getSkillList']);
		// show login form
		...
		const fetchProjectList = async () => {
			...
			await store.dispatch('job/getMatchingJobList', values);
		};
