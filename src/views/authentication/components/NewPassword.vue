<template>
	<div>
		<div class="show_content">
			...
			<div class="mb-2">
				<label class="semibold" for="password">{{ t('fieldList.cpassword') }} </label>
				<input
					type="password"
...
export default {
	...
	setup() {
		...
		const submitForm = handleSubmit(values => {
			values.resetPassword = route.query.resetPassword;
			store
				.dispatch('auth/forgotPasswordRest', {
					...values,
					user_type: localStorage.getItem('changepasstype'),
					email: localStorage.getItem('changepassemail'),
				})
				.then(() => {
					resetForm();
					localStorage.removeItem('changepasstype');
					localStorage.removeItem('changepassemail');

					store.commit('auth/SET_STATE', {
						action: 'forgotPassFlag',
						data: true,
					});
					document.querySelector('.signin_hide').style.display = 'block';
					document.querySelector('.forgot_password_content').style.display = 'none';
					document.querySelector('.forgot_password_show').style.display = 'none';
					document.querySelector('.show_content').style.display = 'none';
