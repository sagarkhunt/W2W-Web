	...
	<div class="invoice__list card-space">
		<form class="theme-input tab-form">
			<div class="invoice-sec">
				<div class="row">
					<div class="payment-pill-header mt">
						<div>
							<h5 class="title-lg">{{ t('workDetails.addWorkDetails') }}</h5>
						</div>
					</div>
					<div class="col-xl-6 mt-4">
						<div class="col-xl-12 col-lg-6 col-12 form-group p-0">
							<label for="addShowInTeam" class="company-titel">{{ t('workDetails.showInTeam') }}</label>
						...
						<div class="pph-left-box">
							<div class="row">
								...
								<div class="col-md-12 form-group">
									...
									<input
										...
										type="text"
										class="form-control"
										:placeholder="t('validation.enterField', { field: 'fieldList.tripName' })"
										@input="tripNameField.handleChange"
										@blur="tripNameField.handleBlur"
										:value="tripNameField.value"
									/>
									<p class="text-danger">
										{{ tripNameField.errorMessage }}
									</p>
								</div>
								<!-- End trip Name -->
							...
							<!-- End Ending Address -->
							<div class="row">
								<span v-if="showTeamField.value" class="row pe-0">
								...
								<label for="firstName" class="company-titel pl-1">{{ t('invoiceList.leInvoice.addInvoice.extras') }} </label>
								<div class="ml-3">
									<div class="row">
										<div class="col-md-4 form-check form-checkbox">
											<input
												...
												type="checkbox"
												id="workTransportedHeavyLoad"
												:checked="transportedHeavyLoad"
												:value="transportedHeavyLoad.value"
												:v-model="transportedHeavyLoad.value"
												@click="onTransported($event)"
											/>
											<label class="form-check-label extras-font checkbox-left-padding white-space" for="workTransportedHeavyLoad">
							...
							<div class="row">
								...
								<div class="col-md-4 col-6">
									...
									<input class="bg-transparent border-0 trip-text w-100" id="addCalculatedtripTime" type="text" :value="Number.parseFloat(calculateTripTime).toFixed(2)" disabled />
								</div>
								...
								<div class="col-md-4 col-6">
									...
									<p class="trip-text d-none" id="totalDistance"></p>
			...
			<div class="invoice-pd detail-sec">
				...
				<div class="invoice-grey-sec">
					<div class="row">
						...
						<div class="col-xl-6 col-lg-6 col-12 form-group">
							...
							></textarea>
							<p class="text-danger">
								{{ workDescription.errorMessage }}
							</p>
						</div>

						<div class="col-xl-3 col-lg-6 col-12 form-group">
							<label for="addTeamHoursPieces" class="company-titel">{{ t('fieldList.hoursPieces') }}</label>
							<div class="form-control d-flex">
								<input
									id="addTeamHoursPieces"
									type="number"
									class="input-area"
						...
						<div class="col-xl-3 col-lg-6 col-12 form-group">
							<label for="addTeamHoursPieces" class="company-titel">{{ t('fieldList.lunchCompensation') }}</label>
							<div>
								<Multiselect
									id="addTeamLunchCompensation"
									class="form-group"
						...
						<div class="col-xl-3 col-md-6 form-group mb-3">
							...
							<div>
								<Multiselect
									id="addTeamFullHalfDayCompensation"
									class="form-group"
			...
			<div class="invoice-pd invoice-receipt">
				<div class="row" v-for="(row, index) in receiptItem" :key="row">
					<div class="d-flex justify-content-between">
						<div class="pph-left">
							<h5 class="title-lg mb-2">{{ t('workDetails.addWorkDetails') }}</h5>
						</div>
					...
					<div class="col-xl-6 col-6 form-group">
						<label for="addTeamPlaceOfPurchase" class="input-title">{{ t('invoiceList.leInvoice.viewInvoice.placeOfPurchase') }}</label>
						<input id="addTeamPlaceOfPurchase" type="text" class="form-control form-grey-input" aria-label="Text input with dropdown button" v-model="row.purchase_place" />
					</div>
					<!-- trip Name -->

					<div class="col-xl-6 col-6 theme-input">
						<label for="teamDateofPurchase" class="input-title">{{ t('invoiceList.leInvoice.viewInvoice.dateOfPurchase') }}</label>
						<v-date-picker
						...
						<div class="">
							<div>
								<div class="input-pera btn-color position-relative">
									<drop-zone @fileDrop="file => handleContractFileChange(file, index)" class="pb-4" :isUploadButton="false" />
			...
			<div class="text-center mt-5 pb-3">
				<ToolTip :title="t('toolTip.submitWorkDetail')">
...
export default {
	...
	setup() {
		...
		document.title = `${t('workDetails.addWorkDetails')} - Way2Work`;
		...
		const selectedTeams = computed(() => {
			return (showTeamField.value = !showTeamField.value);
		});

		watch(
			() => tripStartDateField,
			() => {
				console.log('fdhjsbfghdshjg');
			},
		);
		...
		const submitForm = handleSubmit(async values => {
			...
			values.trip_start_date = values.trip_start_date && moment(values.trip_start_date).format('YYYY/MM/DD');
			values.trip_end_time = EndTime;
			values.trip_end_date = values.trip_end_date && moment(values.trip_end_date).format('YYYY/MM/DD');
			...
			receiptItem.value.map(function(item) {
				item.purchase_date = item.purchase_date == null ? null : moment(item.purchase_date).format('YYYY/MM/DD');
			});
			values.receipts = receiptItem.value || [];
			...
			await store
				.dispatch('workDetail/addWorkDetails', values)
				.then(({ data }) => {
					...
					if (route.params.workDairies) {
						...
						workDetails = workDetails.filter(wd => wd.id !== data.id);
						...
						if (route.params.workDairies == 'ADD') {
							...
							localStorage.setItem('invoiceWorkDairiesDetails', JSON.stringify(data));
							router.push({
								name: 'Edit Invoice',
								params: { id },
							});
