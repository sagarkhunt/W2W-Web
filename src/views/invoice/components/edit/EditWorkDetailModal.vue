												...
												<div class="pph-left-box">
													<div class="row">
															...
															<div class="row">
																<div class="col-md-4 form-check form-checkbox">
																	<input
																		...
																		type="checkbox"
																		id="editTransportedHeavyLoad"
																		:checked="transportedHeavyLoad"
																		:value="transportedHeavyLoad.value"
																		:v-model="transportedHeavyLoad.value"
																		@click="onTransported($event)"
																	/>
																	<label class="form-check-label extras-font checkbox-left-padding white-space" for="editTransportedHeavyLoad">
														...
														<div class="col-md-4 col-6">
															...
															<p class="trip-text" id="tripTime" disabled>{{ calculateTripTime }}</p>
														</div>
										...
										<div class="invoice-grey-sec">
											<div class="row">
												...
												<div class="col-md-4 form-group">
													...
													<div>
														<Multiselect
															id="editFullHalfDayCompensation"
															class="form-group"
...
export default {
	...
	setup(props, { emit }) {
		...
		const otherCompensationField = reactive(useField('other_compensation', undefined, { initialValue: null }));
		...
		const submitForm = handleSubmit(async values => {
			...
			workDetailData.fullHalfDayCompensationId = values.day_compensation == 0 ? null : values.day_compensation;
			workDetailData.lunch_compensation = values.lunch_compensation == 0 ? null : values.lunch_compensation;
			workDetailData.lunchCompensationId = values.lunch_compensation == 0 ? null : values.lunch_compensation;
			...
			workDetailData.date = values.date == null ? new Date() : moment(values.date).format('YYYY/MM/DD');
		...
		const originStart = place => {
			start.value = place.formatted_address ?? null;
		};
		// mid point address
		...
		const destionationEnd = place => {
			end.value = place.formatted_address ?? null;
		};
		...
		const fetchInvoiceWorkDairies = () => {
			...
			lunchCompensationField.value = dataWorkDetail.lunchCompensationId || 0;
			dayCompensationField.value = dataWorkDetail.dayCompensationId || 0;
			otherCompensationField.value = dataWorkDetail.other_compensation || 0;
			store.commit('workDetail/SET_STATE', {
				action: 'totalKilloMeter',
				data: Number(dataWorkDetail.distance).toFixed(2),
			});
			...
			destionationEnd(dataWorkDetail.trip_start_date ?? '');
			setValues({
				...
				day_compensation: dataWorkDetail.fullHalfDayCompensationId,
				distance: dataWorkDetail.distance,
				total_work: dataWorkDetail.total_work,
				other_compensation: dataWorkDetail.other_compensation,
