<template>
				...
				<div v-for="(item, index) in workDetailList" :key="index">
						...
						<div class="details-content">
							...
							<div class="work-width ml-xl-0">
								<div class="work py-0">
									<input type="text" v-model="item.description"
										@change="e => editHandleContractValue(e, index, 'description')"
										class="form-control" />
								</div>
							</div>
							...
							<div class="contractvalue-width">
								<div class="totalamount">
									...
									<input type="number" min="0" v-model="item.contract_value"
										...
										class="inpult-area" @input="checkInputNuberValue" />
								</div>
							</div>
							...
							<div class="totalamount-width">
								<div class="totalamount">
									...
									<input type="number" min="0" v-model="item.total_work"
										@change="e => editHandleContractValue(e, index, 'total_work')"
										class="inpult-area" v-on:keypress="NumbersOnly" @input="checkInputNuberQty" />
								</div>
							</div>
							...
							<div class="buttons">
								...
								<!-- expand work detail -->
								<ToolTip :title="t('toolTip.expantiate')">
									<div class="down-btn me-1" v-if="index === expandRowIndex" @click="sinkWorkDetails">
										<FontAwesomeIcon :icon="upIcon" />
									</div>
								</ToolTip>

								<ToolTip :title="t('toolTip.tripButton')">
									<div class="down-btn" :class="{ btnDisabled: workDetailBtnDisabled(index) }"
										@click="addWorkDetails(item.id, index, item)">
											...
											<div class="row no-gutters">
												...
												<div class="col-4">
													<ul>
														...
														<li class="dark-box-text lh-lg">
															€
															{{ item.fullHalfDayCompensationAmount || 0 * item.totalDays
																	|| 0
															}}
														</li>
					...
					<div v-else>
						<div class="details-content">
							...
							<div class="hours-width">
								<div class="hours">
									<Multiselect id="contractType" class="form-group" v-model="item.contract_type"
										@change="e => editHandleContractValue({ target: { value: e } }, index, 'contract_type')"
										:options="contractTypeOptions.map(x => ({ ...x, label: t(x.label) }))"
										mode="single" :canDeselect="true" />
								</div>
							...
							<div class="contractvalue-width">
								<div class="totalamount">
									<p>€</p>
									<input type="number" v-model="item.contract_value"
										@change="e => editHandleContractValue(e, index, 'contract_value')"
							...
							<div class="totalamount-width">
								<div class="totalamount">
									...
									<input type="text" v-model="item.total_work"
										@change="e => editHandleContractValue(e, index, 'total_work')"
										class="inpult-area" v-on:keypress="NumbersOnly" @input="checkInputNuberQty" />
								</div>
							</div>
							...
							<div class="buttons">
								...
								<!-- remove work detail -->
								<ToolTip :title="t('toolTip.deleteRecord')">
									<div class="delete-btn" :class="{ btnDisabled: workDetailBtnDisabled(index) }"
										@click="removeWorkDetail(index)">
										<FontAwesomeIcon :icon="trashIcon" />
									</div>
								</ToolTip>
		...
		<div class="attachment-table responsive-table ">
			<div class="mb-3" v-for="(item, index) in workDetailList" :key="index">
				<div class="d-flex align-items-center justify-content-between position-relative collapse-div px-3 pt-3">
					...
					<div class="d-flex expand-daily-table-header">
						<div class="expand-daily-table-header">
							...
							<div class="mx-0">
								<div class="d-flex align-items-center">
									<div class="pe-0 mb-sm-0 mb-2 w-100">
										<div class="product-2 d-flex">
											<span v-if="item.workdetail_createdat_type == 'detailadded'">{{dateFormat(item.workdetail_created_at)}}</span>
											<span v-else-if="item.workdetail_createdat_type == 'detailupdate'">{{item.workdetail_created_at}}</span>
											<span v-else>{{dateNewFormatWork(item.trip_start_date,item.trip_end_date)}}</span>
											<!-- {{ dateFormat(item.workdetail_created_at) }} -->
											<!-- <input type="number" min="0" :v-model="dateFormat(item.created_at)" class="inpult-area" /> -->
										</div>
									</div>
									<!-- actions -->
									<div class="pe-0 ps-lg-2 ms-2 mb-sm-0 mb-2">
										<div class="buttons d-flex align-items-center justify-content-sm-between">
											...
											<ToolTip class="ms-1" :title="t('toolTip.tripButton')">
												<div class="down-btn btnDisabled"
											...
											<ToolTip class="ms-1" :title="t('toolTip.duplicateRecord')">
												<div class="copy-btn me-0 btnDisabled"
								...
								<!-- contract type -->
								<!-- <div class="col-sm-2 col-4 responsive-col-2 pe-0 ps-2 mb-sm-0 mb-2">
									<div class="hours">
										<Multiselect
											id="contractType"
											class="form-group"
								...
								<!-- contract value -->
								<!-- <div class="col-sm-2 col-3 pe-0 ps-2 mb-sm-0 mb-2">
									<div class="product-2 d-flex">
										<p class="mr-2">€</p>
										<input
											type="number"
								...
								<!-- quantity & unit -->
								<!-- <div class="col-sm-2 col-3 pe-0 ps-lg-2 ps-sm-2 ps-0 mb-sm-0 mb-2">
									<div class="product-2 d-flex">
										<div class="mr-2 text-sm">
											<FontAwesomeIcon :icon="closeIcon" />
										</div>
								...
								<!-- actions -->
								<!-- <div class="col-sm-2 col-5 pe-0 ps-lg-2 ps-1 mb-sm-0 mb-2">
									<div class="buttons d-flex align-items-center justify-content-sm-between">
										<ToolTip :title="t('toolTip.duplicateRecord')">
										...
										<ToolTip :title="t('toolTip.tripButton')">
											<div class="down-btn" :class="{ btnDisabled: workDetailBtnDisabled(index) }" @click="addWorkDetails(item.id, index, item)">
												<FontAwesomeIcon :icon="euroSign" />
											</div>
										</ToolTip>
									</div>
					...
					<!-- <div class="collapse-arrow" @click="openTable(index, item)">
						<a>
								...
								<div class="col-12 px-0">
									<div class="work-width ml-xl-0 mb-2">
										<div class="work py-0">
											...
											" v-model="item.description" @change="e => editHandleContractValue(e, index, 'description')"
												class="form-control" />
										</div>
									</div>
								</div>
								<div class="col-sm-8 col-12 pe-0 ps-0 mb-sm-0 mb-2">
									...
									<div class="d-flex align-items-center">
										<div class="hours min-w-150 vat_text">
											<Multiselect id="contractType" class="form-group"
												@change="e => editHandleContractValue({ target: { value: e } }, index, 'vat_value')"
									...
									<div class="row no-gutters">
										...
										<div class="col-4">
											<ul>
												...
												<li class="dark-box-text lh-lg">
													€
													{{ Number(item.distance * invoiceAllownceTracer.distanceAllownce *
															item.totalDays).toFixed(2) || 0
													}}
												</li>
												<span v-if="item.distance">
					...
					<div class="total">
						...
						<div class="text-end margin-top-5">
							...
							<div class="total-excl">
								...
								<p class="text-prize">
									€
									{{ editGrossAfterCompensationDeduct ? editGrossAfterCompensationDeduct : 0 }}
								</p>
							</div>
						...
						<div class="text-end margin-top-5">
							<div class="total-excl">
								<p class="text-total">{{ t('invoiceList.leInvoice.addInvoice.willBePaid') }}</p>
								<p class="text-prize">
									€
	...
	<Suspense v-if="editWorkDeatilsModal">
		<template #default>
			<edit-work-details-modal v-model:isModalShow="editWorkDeatilsModal" :editWorkInfo="editWorkInfo"
				:indexId="indexId" :addNewItem="addNewItem" @modalFlag="updateFlag" @workDetails="workDetails" />
		</template>
		<template #fallback>
			<default-loader />
		</template>
...
import { currencyConverture, dateFormat,dateNewFormatWork } from '@/utils/mixins';
...
export default {
	...
	setup(props) {
		...
		const editGrossAfterCompensationDeduct = ref(null);
		...
		const updateInputByOnBlur = async () => {
			...
			await store
				...
				.then(res => {
					...
					GrossSalary.value = data.gross_salary ?? 0;
					editGrossAfterCompensationDeduct.value = data.gross_after_compensation_deduct ?? 0;
		...
		return {
			...
			editGrossAfterCompensationDeduct,
			TaxbledAmount,
			compensationAmount,
			youWillBePaid,
			invoiceTotal,
