	...
	<div class="model">
		...
		<div class="d-flex justify-content-between align-items-center mb-3 mt-5 ">
			...
			<div class="pph-right d-flex justify-content-end">
				<ToolTip title="Add attachments row">
										...
										<div class="border-padding">
											<div v-if="i == tableIndex" class="table-border">
												...
												<new-tb-tr class="bottom_border">
													...
													<new-tb-td
														><div class="d-flex align-items-center justify-content-center w-10 ms-2">
															<button type="button" class="close-btn" @click="removeRow(index, tr.id)">
																<FontAwesomeIcon :icon="closeIcon" />
															</button></div
													></new-tb-td>
												</new-tb-tr>
...
export default {
	...
	setup(props) {
		...
		const receiptData = computed(() => store.state.invoice?.invoiceInfo?.invoice_files);
		...
		const handleContractFileChange = (file, index) => {
			...
			store.dispatch('uploadImage', formData).then(res => {
				...
				editFormFieldsData.value.receiptFileField.value = receiptItem.value || [];
			});
		};
		...
		const removeRow = (index, id) => {
			if (id) {
				store.dispatch('invoice/deleteInvoiceImage', id).then(() => {
					receiptItem.value.splice(index, 1);
				});
			}

			store.commit('invoice/DELETE_ATTACHMENT', index);
			receiptItem.value.splice(index, 1); 
		};
		...
		watch(receiptData, () => {
			receiptItem.value = receiptData.value.map(x => ({
				id: x.id,
