<template>
	<div class="document__list  card-space container-lg pt-4" id="divToPrint">
		<div>
						...
						<div class="pph-left-box large-table">
							...
							<div class="company-content">
								...
								<div class="company-text">
									...
									<p class="company-value">
										{{ seekerInvoiceDetail.invoice_company && seekerInvoiceDetail.invoice_company.cityReference && seekerInvoiceDetail.invoice_company.cityReference.city_name }}
									</p>
								</div>
						...
						<div class="red-box">
							...
							<div>
								...
								<div class="mb-2">
									...
									<p class="text-md-fw">
										{{ seekerInvoiceDetail?.ref_number || 'Not assigned yet' }}
									</p>
								</div>
			...
			<div>
					...
					<new-data-table :data="seekerInvoiceDetail.worked_hours || []" sst class="large-table ">
						...
						<template #data="scopedData" ref="tableBody">
							<new-tb-tr class="invoice__row" :key="i" v-for="(tr, i) in scopedData.data">
								...
								<new-tb-td class="text-center">€ {{ seekerInvoiceDetail.mark_as_wrong ? -(tr.contract_value) : tr.contract_value || '-' }}</new-tb-td>

								<new-tb-td class="text-center">{{ seekerInvoiceDetail.mark_as_wrong ? -(tr.total_work) : tr.total_work || '-' }}</new-tb-td>

								<new-tb-td class="text-center">{{ seekerInvoiceDetail.mark_as_wrong ? -(seekerInvoiceDetail.vat) : seekerInvoiceDetail.vat || '-' }}</new-tb-td>

								<new-tb-td>€ {{ seekerInvoiceDetail.mark_as_wrong ? -(tr.contract_value * tr.total_work) : tr.contract_value * tr.total_work }}</new-tb-td>

								<new-tb-td>€ {{ seekerInvoiceDetail.mark_as_wrong ? -((tr.contract_value * tr.total_work * seekerInvoiceDetail.vat) / 100) : (tr.contract_value * tr.total_work * seekerInvoiceDetail.vat) / 100 }} </new-tb-td>
								<new-tb-td>€ {{ seekerInvoiceDetail.mark_as_wrong ? -(tr.contract_value * tr.total_work + (tr.contract_value * tr.total_work * seekerInvoiceDetail.vat) / 100).toFixed(2) :(tr.contract_value * tr.total_work + (tr.contract_value * tr.total_work * seekerInvoiceDetail.vat) / 100).toFixed(2) }}</new-tb-td>
							</new-tb-tr>
						</template>
					</new-data-table>
				</div>
				...
				<div class="min_width_auto">
					<new-data-table :data="seekerInvoiceDetail.worked_hours || []" sst class="responsive-table mt-4">
						<template #data="scopedData" ref="tableBody">
							<new-tb-tr class="invoice__row expand-table " :key="index" v-for="(tr, index) in scopedData.data">
								<div class="d-flex align-items-center justify-content-between collapse-div" @click="openSeekerTable(index, item)">
									<div class="d-flex">
										<div>
											<div>
												{{ tr.date || '-' }}
									...
									<div>
										<a>
											<FontAwesomeIcon :icon="arrowDown" v-if="newTableIndex !== index" class="arrowIcon" />
										</a>

										<a>
											<FontAwesomeIcon :icon="arrowUp" v-if="newTableIndex === index" class="arrowIcon" />
										</a>
									</div>
								</div>

								<div class="border-padding">
									<div v-if="index == newTableIndex" class="table-border">
										<new-tb-tr>
										...
										<new-tb-tr>
											<new-tb-th>{{ t('invoiceList.leInvoice.addInvoice.vat') }}</new-tb-th>
											<new-tb-td>€ {{ seekerInvoiceDetail.mark_as_wrong ? -((tr.contract_value * tr.total_work * seekerInvoiceDetail.vat)) : (tr.contract_value * tr.total_work * seekerInvoiceDetail.vat) / 100 }}</new-tb-td>
										</new-tb-tr>

										<new-tb-tr>
											<new-tb-th>{{ t('workDetails.total') }}</new-tb-th>
											<new-tb-td>€ {{ seekerInvoiceDetail.mark_as_wrong ? -((tr.contract_value * tr.total_work + (tr.contract_value * tr.total_work * seekerInvoiceDetail.vat))) :(tr.contract_value * tr.total_work + (tr.contract_value * tr.total_work * seekerInvoiceDetail.vat) / 100 || '-').toFixed(2) }}</new-tb-td>
										</new-tb-tr>
									</div>
								</div>
							</new-tb-tr>
				...
				<div class="overflow-auto">
					<div class="bottom-header d-lg-flex align-items-center justify-content-between mb-4 gap-5">
						...
						<div class="d-sm-flex align-items-center ml-md-auto flex-md-wrap flex-wrap gap-4">
							<div class="text-white mb-sm-0 mb-2">
								...
								<p class="text-sm-fw">
									€ {{ seekerInvoiceDetail.mark_as_wrong ? -Number(seekerInvoiceDetail.gross_salary).toFixed(2) : Number(seekerInvoiceDetail.gross_salary).toFixed(2) || '-' }}
								</p>
							</div>
							...
							<div class="text-white mb-sm-0 mb-2">
								...
								<p class="text-sm-fw">
									€ {{ seekerInvoiceDetail.mark_as_wrong ? -Number(seekerInvoiceDetail.tax_amount).toFixed(2) : Number(seekerInvoiceDetail.tax_amount).toFixed(2) || 0.0 }}
								</p>
							</div>
								...
								<p class="text-sm-fw">
									...
									{{
										seekerInvoiceDetail.mark_as_wrong
											? -Number(seekerInvoiceDetail.compensation_amount).toFixed(2)
											: Number(seekerInvoiceDetail.compensation_amount).toFixed(2) || '-'
									}}
								</p>
							</div>
							...
							<div class="text-white">
								...
								<p class="text-sm-fw">
									€ {{ seekerInvoiceDetail.mark_as_wrong ? -Number(seekerInvoiceDetail.sub_total).toFixed(2) : Number(seekerInvoiceDetail.sub_total).toFixed(2) || '-' }}
								</p>
							</div>
						</div>
					</div>
				...
				<div class="overflow-auto">
					<div class="bottom-header d-lg-flex align-items-center justify-content-between mb-4 gap-5">
						...
						<div class="d-sm-flex align-items-center ml-md-auto flex-md-wrap flex-wrap">
							<div class="mr-5 text-white mb-sm-0 mb-2">
								<p class="text-sm-fw">{{ t('invoiceList.leInvoice.addInvoice.invoiceTotal') }}</p>
								<p class="text-sm-fw">
									€
									{{
										seekerInvoiceDetail.mark_as_wrong
											? -Number(seekerInvoiceDetail.company_pay_amount ?? 0).toFixed(2)
											: Number(seekerInvoiceDetail.company_pay_amount ?? 0).toFixed(2) || 0.0
									}}
								</p>
							</div>
							...
							<div class="mr-5 text-white mb-sm-0 mb-2">
								...
								<p class="text-sm-fw">€ {{ seekerInvoiceDetail.mark_as_wrong ? -seekerInvoiceDetail.vat_amount : seekerInvoiceDetail.vat_amount || 0 }}</p>
							</div>
						</div>
					</div>
				</div>
				...
				<new-data-table :data="seekerInvoiceDetail.invoice_files || []" sst class="large-table">
					...
					<template #data="scopedData" ref="tableBody">
						<new-tb-tr class="invoice__row" :key="i" v-for="(tr, i) in scopedData.data">
							...
							<new-tb-td>
								<div class="d-flex align-items-center">
									<ToolTip :title="t('toolTip.ViewAttachment')">
				...
				<div class="min_width_auto">
					<new-data-table :data="seekerInvoiceDetail.invoice_files || []" sst class="responsive-table attachment-table mt-4">
						<template #data="scopedData" ref="tableBody">
							...
							<div class="d-flex">
								...
								<div class="p-2 d-flex" style="width:55%;border-bottom:2px solid #000;">
									...
									<div class="p-2 d-flex" style="width:50%; ">
										...
										<div class="d-flex  align-items-center">€ {{ seekerInvoiceDetail.mark_as_wrong
											? -(seekerInvoiceDetail.sub_total) : seekerInvoiceDetail.sub_total }}</div>
									</div>
								</div>
							</div>
						</div>
...
import { faChevronDown, faChevronUp, faEdit, faInfo, faLongArrowAltLeft, faLongArrowAltRight, faEye, faClock } from '@fortawesome/free-solid-svg-icons';
import { useRoute } from 'vue-router';
import moment from 'moment';
import { invoiceAllownceTracer } from '@/utils/enums.js';
...
export default {
	...
	setup() {
		...
		const transportedHeavyloadsVal = ref(0);
		const drivenInRoughTerrainVal = ref(0);
		const vehicleSubclassVal = ref(0);
		// gloal store
		 ...
		 */
		onMounted(() => {
			store.dispatch('invoice/viewJobseekerInvoice', route.params.id).then(res => {
				res.data.worked_hours.forEach(element => {
					if (element.transported_heavyloads) {
						transportedHeavyloadsVal.value = element.distance * invoiceAllownceTracer.transported_heavyloads;
					}
					if (element.driven_in_rough_terrain) {
						drivenInRoughTerrainVal.value = element.distance * invoiceAllownceTracer.driven_in_rough_terrain;
					}
					if (element.vehicle_subclass) {
						vehicleSubclassVal.value = element.distance * invoiceAllownceTracer.vehicle_subclass;
					}
					const noVatTotal = element.contract_value * element.total_work - (element.contract_value * element.total_work * res.data.vat) / 100;
					const vatTotal = (element.contract_value * element.total_work * res.data.vat) / 100;

					element.oneWorkDetailTotal = noVatTotal + vatTotal;
				});
			});
		});
		...
		const seekerInvoiceDetail = computed(() => store.getters['invoice/getInvoiceSeekerDetails']);
		// view document
		...
		watch(seekerInvoiceDetail, () => {
			...
			seekerInvoiceDetail.value.worked_hours.forEach(element => {
				...
				totalNoVatAmount.value += element.contract_value * element.total_work;
				totalVatAmount.value += (element.contract_value * element.total_work * vatValue) / 100;
				totalWorkAmount.value += element.contract_value * element.total_work + (element.contract_value * element.total_work * vatValue) / 100;
			});
		});
		...
		return {
			...
			totalVatAmount,
			totalWorkAmount,
			transportedHeavyloadsVal,
			drivenInRoughTerrainVal,
			vehicleSubclassVal,
