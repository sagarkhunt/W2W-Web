			...
			<div class="daily-work-col">
				...
				<div v-for="(tMem, index) in teamMembers" :key="index">
					<div class="details-content text-center">
						...
						<div class="role-member">
							<span v-if="getTeamDetails.createdById === tMem?.seekerId">
						...
						<div class="email-address mr-2 text-break">
							<div>
								<p>€ {{ Number(tMem.TotalWorkValue).toFixed(2) }}</p>
							</div>
						</div>
					...
					<div class="opne-toggle p-3 border-top border-bottom" v-if="index === expandRowIndex">
							...
							<div class="row no-gutters">
								<div class="details-content border rounded-bottom" v-for="(tMW, wI) in tMem?.seeker?.work_hours || []" :key="wI">
									...
									<div class="description-width">
										...
										<span v-else class=""> {{ t('invoiceList.noDescription') }} </span>
									</div>
						...
						<!-- work detail stats - le -->
						<div class="overflow-auto">
							<div class="bottom-header d-flex align-items-center justify-content-between mt-3">
								<div class="pph-left">
									<h5 class="text-white">{{ t('invoiceList.leInvoice.addInvoice.le') }}</h5>
								</div>
								<div class="d-flex align-items-center ml-md-auto">
									<div class="mr-5 text-white">
										<p class="text-md-fw">{{ t('invoiceList.payableAmount') }}</p>
										<p class="text-md-fw">€ {{ remainingWorkValue(index).toFixed(2) || 0 }}</p>
									</div>

									<div class="mr-5 text-white">
										<p class="text-md-fw">{{ t('workDetails.taxableAmount') }}</p>
										<p class="text-md-fw">€ {{ calculateTaxableAmount(index).toFixed(2) || 0 }}</p>
									</div>

									<div class="mr-5 text-white">
										<p class="text-md-fw">{{ t('workDetails.totalCompensation') }}</p>
										<p class="text-md-fw">€ {{ Number(tMem.TotalCompensation).toFixed(2) || 0 }}</p>
									</div>

									<div class="text-white">
										<p class="{{'text-md-fw'|translate}}">{{ t('workDetails.total') }}</p>
										<p class="text-md-fw">€ {{ calculateTotalOfSeeker(index).toFixed(2) || 0 }}</p>
									</div>
								</div>
							</div>
						</div>
							...
							<div class="col-lg-7 col-8 ml-auto p-0">
								<div class="total mt-3">
									<div class="total-excl">
										...
										<p class="text-prize">€ {{ remainingWorkValue(index).toFixed(2) || 0 }}</p>
									</div>

									<div class="text-end margin-top-5">
										<div class="total-excl">
											<p class="text-total">{{ t('workDetails.totalCompensation') }}</p>
											<p class="text-prize">€ {{ tMem.TotalCompensation || 0 }}</p>
										</div>
									</div>
									<div class="text-end margin-top-5">
										<div class="total-excl">
											<p class="text-total">{{ t('invoiceList.leInvoice.addInvoice.grossSalary') }}</p>
											<p class="text-prize">€ {{ calculateGrossSalary(index).toFixed(2) || 0 }}</p>
										</div>
									...
									<div class="text-end margin-top-5">
										<div class="total-excl">
											...
											<p class="text-prize">€ {{ calculateHealthInsurance(index).toFixed(2) || 0 }}</p>
										</div>
										...
										<div class="payment_table">
											...
											<new-tb-tr>
												...
												<new-tb-td>
													<div class="email-address mr-2 text-break">
														<div>
															<p class="total-text">€ {{ Number(tMem.TotalWorkValue).toFixed(2) }}</p>
														</div>
													</div>
												</new-tb-td>
											</new-tb-tr>
											...
											<new-tb-tr>
												...
												<new-tb-td>
													<div class="payment">
														<div>
															<p class="total-text">€ {{ calculateTotalOfSeeker(index).toFixed(2) }}</p>
														</div>
													</div>
												</new-tb-td>
											</new-tb-tr>
											...
											<div class="overflow-auto">
												<div class="bottom-header d-lg-flex align-items-center justify-content-between my-3 gap-5">
													...
													<div class="d-sm-flex align-items-center ml-md-auto gap-4">
														<div class="mb-sm-0 mb-2 text-white">
															<p class="text-sm-fw">
																{{ t('invoiceList.payableAmount') }}
															</p>
															<p class="text-sm-fw">€ {{ remainingWorkValue(index).toFixed(2) || 0 }}</p>
														</div>
														...
														<div class="text-white">
															<p class="text-sm-fw">{{ t('workDetails.total') }}</p>
													...
													<div class="total mt-3">
														...
														<div class="text-end margin-top-5">
															<div class="total-excl">
																...
																<p class="text-prize">
																	€
																	{{ calculateGrossSalary(index).toFixed(2) || 0 }}
																</p>
															</div>
															<span class="text-xs d-block">({{ t('invoiceList.leInvoice.addInvoice.vatAddedVal') }})</span>
														</div>
...
export default {
	...
	setup(props) {
		 ...
		 */
		// get team members
		const teamMembers = computed(() => JSON.parse(JSON.stringify(store.getters['teamInvoice/getTeamMembersFromTeamImvoice'])));
		// get team details
		const getTeamDetails = computed(() => store.getters['teamInvoice/getTeamInvoiceDetail']);

		// get team payment sharing
		const getTeamPaymentSharing = computed(() => store.state.teamInvoice.paymentSharing);

		// calculate remaining work value of seeker
		const remainingWorkValue = computed(() => index => store.getters['teamInvoice/calculateRemainingWorkValueOfSeeker'](index));

		// calculate gross salary of seeker
		const calculateGrossSalary = computed(() => index => store.getters['teamInvoice/calculateRemainingWorkValueOfSeeker'](index));

		// calculate taxable amount of seeker
		const calculateTaxableAmount = computed(() => index => store.getters['teamInvoice/calculateTaxableAmountOfSeeker'](index));

		// calculate hours amount after tax of seeker
		const calculateHoursAmountAfterTax = computed(() => index => store.getters['teamInvoice/calculateHourAmountAfterTaxableAmountOfSeeker'](index));

		// calculate misb commision of seeker
		const calculateMISBCommision = computed(() => index => store.getters['teamInvoice/calculateMISBCommisionOfSeeker'](index));

		// calculate Health Insurance of seeker
		const calculateHealthInsurance = computed(() => index => store.getters['teamInvoice/calculateHealthInsuranceOfSeeker'](index) || 0);
		// calculate Accident Insurance of seeker
		const calculateAccidentInsurance = computed(() => index => store.getters['teamInvoice/calculateAccidentInsuranceOfSeeker'](index) || 0);

		// calculate w2w commision of seeker
		const calculateW2WCommision = computed(() => index => store.getters['teamInvoice/calculateW2WCommisionOfSeeker'](index, props.vat));

		// calculate quick pay commision of seeker
		const calculateQuickPayCommision = computed(() => index => store.getters['teamInvoice/calculateQuickPayCommisionOfSeeker'](index, props.vat, props.quickPayStat));

		// calculate commision amount
		const calculateCommisionAmount = computed(() => index => store.getters['teamInvoice/calculateCommisionAmountOfSeeker'](index));

		// calculate total of seeker
		const calculateTotalOfSeeker = computed(() => index => store.getters['teamInvoice/calculateTotalOfSeeker'](index));

		// calculate share amount and share price
		const calculateShareAmountAndSharePrice = computed(() => store.getters['teamInvoice/calculateShareAmountAndSharePercentage']);
		...
		const leftToShareAmount = computed(() => {
			...
			const totalWorkValue = teamMembers.value.reduce((acc, item) => acc + Number(item.TotalWorkValue), 0);
			const TootalSum = Math.round((Number(totalWorkValue) - Number(totalAllocatedAmount) + Number.EPSILON) * 10 ** 2) / 10 ** 2;
			return Number(TootalSum || 0.0);
		});

		const finalTotal = computed(() => {
			const initObj = { grossSalary: 0, taxbledAmount: 0, compensationAmount: 0, youWillBePaid: 0, invoiceTotal: 0, vatTotal: 0 };
			return (
				store.state.teamInvoice?.teamWorkDetailList?.team_members?.reduce((acc, item, index) => {
					acc.grossSalary += store.getters['teamInvoice/calculateGrossSalaryOfSeeker'](index);
					acc.taxbledAmount += store.getters['teamInvoice/calculateTaxableAmountOfSeeker'](index);
					acc.compensationAmount += Number(item.seeker_total_compensation);
					acc.youWillBePaid += store.getters['teamInvoice/calculateTotalOfSeeker'](index);
					acc.invoiceTotal += (Number(item.seeker_total_worked_value) * (100 + Number(props.vat))) / 100;
					acc.vatTotal += (Number(item.seeker_total_worked_value) * Number(props.vat)) / 100;
					return acc;
				}, initObj) || initObj
			);
		});
