					...
					<div class="red-wrapper">
						...
						<div class="red-box">
							...
							<div class="mb-2" v-if="seekerTeamInvoiceDetail.team_invoice_company && seekerTeamInvoiceDetail.team_invoice_company.reg_number">
								<h5 class="company-value title-lg">{{ t('invoiceList.leInvoice.viewInvoice.companyID') }}</h5>
								<p class="text-md-fw">
									{{ seekerTeamInvoiceDetail.team_invoice_company && seekerTeamInvoiceDetail.team_invoice_company.reg_number }}
								</p>
							</div>
			...
			<div class="mt-4">
				<div class="bg-white pb-3 mt-10">
					<div class="employer-details flex justify-between items-center">
						<div class="pph-left">
							<h3 class="title-head">{{ t('invoiceList.leInvoice.viewInvoice.seekerWHList') }}</h3>
						</div>
					</div>
								...
								<div class="daily-work-col">
									...
									<div v-for="(item, index) in seekerTeamInvoiceDetail.invoice_seekers || []" :key="index">
										<div class="details-content text-center">
											...
											<div class="payment">
												<div>
													<p class="text-red">€ {{ seekerTeamInvoiceDetail.mark_as_wrong ? -(item.sub_total.toFixed(2)) :item.sub_total.toFixed(2) }}</p>
												</div>
											</div>
												...
												<div class="bottom-header d-md-flex d-block align-items-center justify-content-between mb-4">
													...
													<div class="d-flex align-items-center ml-md-auto">
														<div class="mr-5 text-white">
															...
															<p class="text-sm-fw">€ {{ seekerTeamInvoiceDetail.mark_as_wrong ? -(item.seeker_company_pay && item.seeker_company_pay.toFixed(2)) : (item.seeker_company_pay && item.seeker_company_pay.toFixed(2)) || 0.0 }}</p>
														</div>

														<div class="mr-5 text-white">
															<p class="text-sm-fw">{{ t('invoiceList.leInvoice.addInvoice.VATAmount') }}</p>
															<p class="text-sm-fw">€ {{ seekerTeamInvoiceDetail.mark_as_wrong ? -(item.worked_value && item.worked_value.toFixed(2)) : (item.worked_value && item.worked_value.toFixed(2)) || '-' }}</p>
														</div>
													</div>
												</div>
											</div>
					...
					<div class="min_width_auto">
						<new-data-table sst :data="seekerTeamInvoiceDetail.invoice_seekers || []" class="attachment-table responsive-table">
												...
												<div class="payment_table">
													...
													<new-tb-tr>
														...
														<new-tb-td>
															<div class="payment">
																<div>
																	<p class="">€ {{ seekerTeamInvoiceDetail.mark_as_wrong ? -(item.sub_total.toFixed(2)) :item.sub_total.toFixed(2) }}</p>
																</div>
															</div>
														</new-tb-td>
													</new-tb-tr>
																...
																<new-tb-td>
																	<div class="contractvalue-width">
																		<p>
																			€
																			{{
																				seekerTeamInvoiceDetail.mark_as_wrong ? -(itemRaw.contract_value * itemRaw.total_work +
				...
				<div class="min_width_auto">
					<new-data-table :data="seekerTeamInvoiceDetail.invoice_files || []" sst class="attachment-table responsive-table mt-4">
						<template #data="scopedData" ref="tableBody">
								...
								<div class="d-flex align-items-center justify-content-between collapse-div" @click="openSeekerTable(i, item)">
									<div class="d-flex">
										<div>
											...
											<div class="d-flex">
												<div>
													{{ t('invoiceList.leInvoice.viewInvoice.placeOfPurchase') }}
												</div>
											</div>
										</div>
									</div>
					...
					<div class="viewinvoicetable">
						...
						<div class="d-flex align-items-center my-3" style="height:72px;">
							<div class="barcode-text">
								<p class="d-flex align-items-center justify-content-center">{{ t('invoiceList.leInvoice.viewInvoice.barcode_here') }}</p>
							</div>
						</div>
					</div>
				</div>
...
export default {
	name: 'CompanyTeamInvoiceSeekerDetail',
	...
	setup() {
		 ...
		 */
		onMounted(() => {
			store.dispatch('teamInvoice/viewCompanyTeamInvoice', route.params.id);
		});
		...
		const seekerTeamInvoiceDetail = computed(() => store.getters['teamInvoice/getTeamInvoiceCompanyDetails']);
